#include <iostream>
#include <string>
using std::cout;//постоянно заменяет
using std::cin;
using std::endl;

void bubble(int* arr, int i) {//сортировка пузырьком
	for (int k = 0; k < i - 1; k++) {
		for (int j = 0; j < i - 2; j++) {
			if (arr[j] > arr[j + 1]) {
				int change = arr[j];
...
void counter(char* arr1, int a) {//сортировка подсчетом
	const int elem = 26;
	int arr[elem] = { 0 };//масив
	cout << "\n";
	for (int i = 0; i < a; i++) {
		int j = arr1[i] - 'a';
		arr[j]++;
...
void merge(int* a, int n)//сортировка слиянием,указатель значением которой является чейка памяти, если перед то разименовыванием, после указатель
{
	...
	int* c = (int*)malloc(n * sizeof(int));//генерирует размер хранилища типа данных
	int step;
	while (h < n)
	{
		step = h;
